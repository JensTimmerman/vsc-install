...
class vsc_setup(object):
    ...
    def get_name_url(filename=None, version=None, license_name=None):
        ...
        if filename is None:
            git_config = os.path.join(vsc_setup.REPO_BASE_DIR, '.git', 'config')
            pkg_info = os.path.join(vsc_setup.REPO_BASE_DIR, 'PKG-INFO')
    ...
    def rel_gitignore(paths, base_dir=None):
        ...
        """
        if not base_dir:
            base_dir = vsc_setup.REPO_BASE_DIR
    ...
    def files_in_packages(excluded_pkgs=None):
        ...
        offset = len(vsc_setup.REPO_LIB_DIR.split(os.path.sep))
        for root, _, files in os.walk(vsc_setup.REPO_LIB_DIR):
            ...
            if '__init__.py' in files or package in excluded_pkgs:
                ...
                res['packages'][package] = vsc_setup.rel_gitignore([os.path.join(root, f) for f in files])
    ...
    class vsc_sdist(sdist):
        ...
        def _recopy(self, base_dir, *paths):
            ...
            if hasattr(os, 'link') and os.path.exists(dest):
                ...
                os.unlink(dest)
                self.copy_file(os.path.join(vsc_setup.REPO_BASE_DIR, *paths), dest)
        ...
        def make_release_tree(self, base_dir, files):
            ...
            for fn in [LICENSE, README]:
                self.copy_file(os.path.join(vsc_setup.REPO_BASE_DIR, fn), os.path.join(base_dir, fn))
    ...
    class vsc_egg_info(egg_info):
        ...
        def finalize_options(self, *args, **kwargs):
            ...
            cleanup = []
            if not os.path.exists(vsc_setup.REPO_LIB_DIR):
                log.warn('vsc_egg_info create missing %s (will be removed later)' % vsc_setup.REPO_LIB_DIR)
                os.mkdir(vsc_setup.REPO_LIB_DIR)
                cleanup.append(vsc_setup.REPO_LIB_DIR)
    ...
    class VscTestCommand(TestCommand):
        ...
        TEST_LOADER = 'shared_setup:vsc_setup.VscScanningLoader'
        ...
        def reload_modules(self, package, remove_only=False, own_modules=False):
            ...
            def candidate(modulename):
                ...
                if own_modules:
                    is_own_module = modulename in vsc_setup.files_in_packages()['modules']
                else:
                    is_own_module = True
        ...
        def setup_sys_path(self):
            ...
            if os.path.isdir(vsc_setup.REPO_TEST_DIR):
                sys.path.insert(0, vsc_setup.REPO_TEST_DIR)
            else:
                raise Exception("Can't find location of testsuite directory %s in %s" %
                                (DEFAULT_TEST_SUITE, vsc_setup.REPO_BASE_DIR))

            # insert REPO_BASE_DIR, so import DEFAULT_TEST_SUITE works (and nothing else gets picked up)
            sys.path.insert(0, vsc_setup.REPO_BASE_DIR)

            # make sure we can import the script as a module
            if os.path.isdir(vsc_setup.REPO_SCRIPTS_DIR):
                sys.path.insert(0, vsc_setup.REPO_SCRIPTS_DIR)

            # insert lib dir before newly inserted test/base/scripts
            sys.path.insert(0, vsc_setup.REPO_LIB_DIR)

            # force __path__ of packages in the repo (to deal with namespace extensions)

            packages = vsc_setup.files_in_packages()['packages']
    ...
    def generate_scripts(extra=None, exclude=None):
        ...
        res = []
        if os.path.isdir(vsc_setup.REPO_SCRIPTS_DIR):
            res = vsc_setup.rel_gitignore(glob.glob("%s/*" % vsc_setup.REPO_SCRIPTS_DIR))
    ...
    def parse_target(target, urltemplate=None):
        ...
        # license info
        lic_name, lic_classifier = vsc_setup.get_license()
        ...
        if 'name' not in target:
            ...
            target.update(vsc_setup.get_name_url(version=target['version'], license_name=lic_name))
        ...
        # Readme are required
        readme = os.path.join(vsc_setup.REPO_BASE_DIR, README)
        ...
        if vsc_scripts:
            candidates = vsc_setup.generate_scripts()
            if candidates:
