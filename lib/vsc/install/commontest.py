...
class CommonTest(TestCase):
    ...
    def test_import_packages(self):
        ...
        for pkg in vsc_setup().generate_packages(extra=self.EXTRA_PKGS, exclude=self.EXCLUDE_PKGS):
            self._import(pkg)

            if self.CHECK_HEADER:
                for fn in vsc_setup.files_in_packages()['packages'][pkg]:
                    self.assertFalse(check_header(os.path.join(vsc_setup.REPO_BASE_DIR, fn), script=False, write=False),
                                     msg='check_header of %s' % fn)

    def test_import_modules(self):
        """Try to import each module"""
        for modname in vsc_setup().generate_modules(extra=self.EXTRA_MODS, exclude=self.EXCLUDE_MODS):
    ...
    def test_importscripts(self):
        ...
        for scr in vsc_setup.generate_scripts(extra=self.EXTRA_SCRIPTS, exclude=self.EXCLUDE_SCRIPTS):
            if not scr.endswith('.py'):
                continue
            self._import(os.path.basename(scr)[:-len('.py')])

            if self.CHECK_HEADER:
                self.assertFalse(check_header(os.path.join(vsc_setup.REPO_BASE_DIR, scr), script=True, write=False),
    ...
    def test_prospector(self):
        ...
        sys.argv.append(vsc_setup.REPO_BASE_DIR)
